...
import io # to facilitate data upload as byte
...
if selected == "Bulk Data Processing":
    ...
    default_path = r"C:\\Users\\Fanny\\Downloads\\"
    ...
    if email and uploaded_file is not None:
        ...
        if st.button(label="Submit"):
            ...
            payload = {'email': email}
            files = [
                ('csv_file', (uploaded_file.name, open(filename, 'rb'), 'text/csv'))
            ]
            #files = [
                    #(field_name, (file_name, file_object, content type))
                    #("csv_file", open(uploaded_file, 'rb'), 'text/csv')
            #]
            #files = [{"csv_file": bytes_data}]
            #files = [{"csv_file": uploaded_file}]
            #files = [{"csv_file" : f"C:\\Users\\Fanny\\Downloads\\raw_100.csv"}]
            #files = [{uploaded_file.name, file, 'text/csv'}]
            # print(st.text(payload))
            # print(st.text(files))
            headers = {}
